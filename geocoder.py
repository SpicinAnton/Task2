...
def get_ll_span(address, delta):
    toponym = geocode(address, delta='0.005')
    if not toponym:
        return (None, None)
    ...
    dx = abs(float(l) - float(r)) / float(delta)
    dy = abs(float(t) - float(b)) / float(delta)
